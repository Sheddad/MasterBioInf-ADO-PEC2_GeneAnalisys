...
L'objectiu d'aquesta activitat és fer una introducció de forma pràctica a l'ús, d'una banda, del programari Galaxy i, de l'altra, de Bioconductor, el projecte de codi obert que proporciona eines i recursos per a l'anàlisi i visualització de dades en bioinformàtica . En el nostre cas farem servir Bioconductor i R per analitzar dades de NGS.

Per això descarregarem i manipularem unes dades de NGS, les explorarem per saber-ne l'estructura i realitzarem un control de qualitat.

Finalment, presentarem en aquest informe els resultats obtinguts i valorarem la qualitat de les dades així com les diferents tècniques emprades.
...
ens quedarem únicament els gens amb un "log-fold-change" major que 1 i un 
...
dim(topTab)
# Observem els (primers) resultats
head(topTab)
```
...
topTabAnotada <- topTab %>%  
  ...
  arrange(P.Value) %>%
  select(7,8,9, 1:6, 10)
# Visualitzem els (primers) resultatss
head(topTabAnotada)
```
